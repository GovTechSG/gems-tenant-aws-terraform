...
resource "aws_instance" "GEMS_Tenant_Kong_Manager_And_Admin_API" {
  ...
  user_data = <<-EOF
              ...
              while :; do
                curl -sS --fail -o /dev/null -X POST http://localhost:8001/default/services/DemoService/routes -H 'Content-Type: application/x-www-form-urlencoded' -H 'Kong-Admin-Token: Pass1234' -d 'name=DemoRoute&protocols=https&methods=GET' && break
                sleep 1
              done
              EOF
