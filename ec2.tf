...
resource "aws_instance" "GEMS_Tenant_Kong_Manager_And_Admin_API" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sleep 15",
      "curl -X POST http://localhost:8001/default/services -H 'Content-Type: application/x-www-form-urlencoded' -H 'Kong-Admin-Token: ${var.portal_admin_password}' -d 'name=DemoService&protocol=https&host=httpbin.org&port=443'",
      "curl -X POST http://localhost:8001/default/services/DemoService/routes -H 'Content-Type: application/x-www-form-urlencoded' -H 'Kong-Admin-Token: ${var.portal_admin_password}' -d 'name=DemoRoute&protocols=https&methods=GET'"      
      ]
  }
